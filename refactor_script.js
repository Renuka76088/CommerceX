...
filesToProcess.forEach(file => {
  try {
    ...
    if (!content.includes('>Add</button>') && !content.includes('>\\n                    Add\\n                  </button>')) {
       return;
    }
    ...
    const newButton = `<button 
      onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
        const isDecoration = ['decoration', 'wedding', 'event', 'car', 'booth', 'setup', 'studio'].some(word => window.location.pathname.toLowerCase().includes(word));
        if (isDecoration) {
